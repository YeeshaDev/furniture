...
const Sidebar = ({ isSidebarOpen, closeSidebar }) => {
  ...
  const { data: cartProducts, totalAmount,totalItems } = useSelector(
    (state) => state.cart
  );
  ...
  return (
    <div>
      <div
        ...
        className="fixed top-0 right-0 sm:w-[350px] h-full bg-white shadow-lg transition-transform duration-300 ease-in-out overflow-y-auto"
      >
        <div className="border-b mb-4">
          <h1 className="text-2xl p-4">Your Cart ({totalItems})</h1>
        </div>
        ...
        <div className="p-4">
          ...
          {cartProducts.length === 0 ? (
           ...
           <Link to='/shop'>
           <button className="border border-black rounded-2xl px-4 py-3 mt-5">Shop Products</button>
           </Link>
            </div>
          ) : (
            <div>
